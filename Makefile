...
.PHONY: mypy
mypy: deps
	$(PYTHON_CMD) -m mypy streaming

.PHONY: pylint
pylint: deps
	$(PYTHON_CMD) -m pylint streaming
...
.PHONY: coverage
coverage: deps
	$(VENV)/bin/pytest --cov-report html:reports --cov=streaming streaming/
...
watch: deps ## run unit tests continuously
	$(PYTHON_CMD) -m pytest_watcher --ignore-patterns "$(VENV)/*" --now --runner $(VENV_BIN)/pytest streaming
